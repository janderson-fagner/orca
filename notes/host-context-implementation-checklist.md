...
- [~] 2026-06-13: Remaining source-error gap: unavailable hosts and version-skewed runtimes are surfaced, and desktop-owned GitHub/GitLab preflight reports missing auth/tooling, but runtime-owned provider auth/support is not yet emitted per source host. Completing this needs a per-runtime provider diagnostic model rather than reusing the single active preflight result.
- [~] 2026-06-13: Remaining Automation gap: run-host availability and missing project-host setup are handled for manual/internal/external actions, but `Automation.projectId` remains the legacy repo-id compatibility field and internal Orca automations do not yet validate saved `sourceContext` for missing provider auth/tooling or unsupported provider before dispatch.
